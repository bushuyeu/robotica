...
def validate_phmr(path: str | Path) -> list[str]:
    ...
    if not isinstance(people, dict) or len(people) == 0:
        ...
        people = {
            k: v for k, v in data.items() if isinstance(v, dict) and "smplx_world" in v
        }
        if len(people) == 0:
            errors.append(
                "no people found in results (need 'people' dict with 'smplx_world' data)"
            )
            return errors
    ...
    for pid, pdata in people.items():
        ...
        for key, expected_shape in PHMR_EXPECTED_SHAPES.items():
            ...
            if arr.ndim != 2:
                errors.append(
                    f"{prefix}: smplx_world.{key} expected 2D, got {arr.ndim}D"
                )
                continue
            ...
            if expected_shape[1] is not None and arr.shape[1] != expected_shape[1]:
                ...
                if key == "shape" and arr.shape[1] in (10, 16):
                    ...
                    errors.append(
                        f"{prefix}: smplx_world.{key} shape {arr.shape}, expected (N, {expected_shape[1]})"
                    )
            # Check for NaN/Inf
            nan_count = np.sum(~np.isfinite(arr))
            if nan_count > 0:
                errors.append(
                    f"{prefix}: smplx_world.{key} has {nan_count} NaN/Inf values"
                )
        ...
        if len(set(frame_counts)) > 1:
            errors.append(
                f"{prefix}: inconsistent frame counts across keys: {dict(zip(PHMR_EXPECTED_SHAPES.keys(), frame_counts))}"
            )
...
def validate_retarget(path: str | Path) -> tuple[list[str], list[str]]:
    ...
    if nan_count > 0:
        ...
        errors.append(
            f"dof_pos has {nan_count} NaN/Inf values across {nan_frames} frames"
        )
    ...
    for key, expected_cols in [("root_pos", 3), ("root_rot", 4)]:
        if key in data:
            ...
            if arr.ndim != 2 or arr.shape != (n_frames, expected_cols):
                errors.append(
                    f"{key} shape {arr.shape}, expected ({n_frames}, {expected_cols})"
                )
...
def main() -> int:
    if len(sys.argv) != 3 or sys.argv[1] not in VALIDATORS:
        print(
            f"Usage: python -m robotica.validate <{'|'.join(VALIDATORS)}> <path>",
            file=sys.stderr,
        )
        return 1
