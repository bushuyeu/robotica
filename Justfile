...
check:
    ...
    if ! command -v docker &>/dev/null; then
        ...
        || docker image inspect nvgear/gr00t_wbc:latest >/dev/null 2>&1 \
        || sudo docker image inspect gr00t_wbc-deploy-root >/dev/null 2>&1 \
        || sudo docker image inspect nvgear/gr00t_wbc:latest >/dev/null 2>&1; then
